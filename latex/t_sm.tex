...
Following the above rules including some more technical manipulations we can write down the complete amplitude. In case of Figure \ref{fig:feyn:ee_a_ee} the matrix element is given by \cite{griffiths1987iep}

\begin{equation}
	\mathcal{M} = -\frac{g_e^2}{(p_1 + p_2)^2} (\bar u(3) \gamma^\mu v(4)) (\bar v(2) \gamma_\mu u(1)),
\end{equation}
where [SKRIV HVAD u OG v ER!]. Now that we have written down the amplitude for our process, the next step is to calculate the actual cross section for the process. To do this we use Fermi's Golden Rule.
...
The previous discussion illustrated that using very simple arguments, like local gauge invariance, we can, when plugging in the particle wave functions and their masses\footnote{Something that is not predicted per-se.}, calculate an amazing amount of physical results. Results that can be tested almost directly in experiments like the LHC at CERN. We will come back to the discussion of testing things experimentally later.
...
But the loop type diagrams as the one in Figure \ref{fig:feyn:ee_a_ee_2} poses another, more fundamental problem. Just as we had to sum over all possible diagrams, we have to integrate over all possible momenta $p$ of the virtual electron/positron pair in the loop. This integral turns out to depend mainly on $1 / p^4$ and the volume element $p^3$, so we end up with an integral of the form \cite{griffiths1987iep}
\begin{equation}
