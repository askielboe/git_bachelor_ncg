...
Using these diagrams Feynman were able to greatly simplify how we think about and calculate interactions in the SM. Having drawn a diagram as in \ref{fig:feyn:uu_a_mm} we now want calculate the probability for this process to happen. Given the diagram one can write down the amplitude using so called \emph{Feynman rules} associated with the different parts of the diagram\footnote{For a list of Feynman rules see appendix C in \cite{zee2005qft}.}. We won't go in to detail with writing down the complete feynman rules for \ref{fig:feyn:uu_a_mm}, but we can learn a lot by looking at the vertices.
