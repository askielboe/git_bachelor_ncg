...
\begin{equation}
	\alpha = g_{\textrm{\tiny{QED}}} = \frac{e^2}{4\pi\varepsilon_0\hbar c},
\end{equation}
...
\begin{equation}
	 g_{\textrm{\tiny{QED}}} \approx \frac{1}{137}.
\end{equation}
...
But the loop type diagrams as the one in Figure \ref{fig:feyn:uu_a_mm_2} poses another, more fundamental problem. Just as we had to sum over all possible diagrams, we have to integrate over all possible momenta $p$ of the virtual electron/positron pair in the loop. This integral turns out to depend mainly on $1 / p^4$ and the volume element $p^3$, so we end up with an integral of the form
\begin{equation}
	\int_0^\infty \frac{1}{p^4}p^3 dp,
\end{equation}
